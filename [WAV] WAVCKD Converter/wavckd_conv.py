...
wavfile = openFile.filename = filedialog.askopenfilename(initialdir=str(pathlib.Path().absolute()), title="Select your WAV file", filetypes=[("WAV (.wav)", "*.wav")] )
...
with open(wavfile, "rb+") as wav:
        ...
        with open(audio + "/" + CodeName.lower() + ".wav.ckd", "wb+") as ckd:##It creates the wav.ckd
